...
export async function POST(request: NextRequest) {
  try {
    ...
    // Buscar progreso existente
    const existingProgress = await directus.request(
      readItems('practice_progress', {
        filter: {
          user_id: { _eq: userInfo.id },
          practice_slug: { _eq: body.practice_slug },
        },
        limit: 1,
      })
    );
    ...
    if (existingProgress.length > 0) {
      ...
      if (body.completion_percentage !== undefined) {
        updateData.completion_percentage = Math.max(
          existing.completion_percentage || 0,
          body.completion_percentage
        );
      }
      if (body.score !== undefined) {
        updateData.best_score = Math.max(
      ...
      // Marcar primera vez completado si aplica
      if (body.status === 'completed' && !existing.first_completed_at) {
        updateData.first_completed_at = currentTime;
      }

      const updatedProgress = await directus.request(
        updateItem('practice_progress', existing.id, updateData)
      );

      return NextResponse.json(
        {
          success: true,
          practice_progress_id: updatedProgress.id,
          action: 'updated',
        },
        { status: 200 }
      );
      ...
      const newProgress = await directus.request(
        createItem('practice_progress', progressData)
      );

      return NextResponse.json(
        {
          success: true,
          practice_progress_id: newProgress.id,
          action: 'created',
        },
        { status: 201 }
      );
